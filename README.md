...
- Managed toggles for max effort, nonessential traffic, cc co-author attribution, auto mode, and auto theme.
...
CC Provider 是一个用于切换 Claude Code 提供商配置的 VS Code 扩展。它把常见的 provider、base URL、模型名、Token 和相关开关放到左侧侧栏里，避免每次手动打开 `~/.claude/settings.json` 修改 JSON。
...
- 提供 max effort、禁用非必要流量、去除 cc co-author 声明、默认 auto mode、主题 auto 等托管开关。
