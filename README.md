...
- Manual usage area for provider-reported data: DeepSeek balance with a platform usage link, experimental Zhipu GLM Coding Plan usage with a plan usage link, and a Xiaomi MiMo console link where no public usage API is available.
- Custom providers can optionally configure a usage link that opens the provider's own console.
...
- 提供手动用量区域，仅展示厂商真实返回的数据：DeepSeek 余额和官网用量链接、实验性的 Zhipu GLM Coding Plan 用量与套餐用量链接，以及没有公开查询 API 时的 Xiaomi MiMo 控制台入口。
- 自定义 Provider 可选择配置一个用量链接，用于打开对应厂商自己的控制台。
